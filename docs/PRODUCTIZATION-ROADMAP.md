...
Gate：仓库源树的精确角色、名称和 7 Skill 物理集合已由用户确认并实现；项目级 `.agents/skills`、`skills/list` 和 `$skill-name` 显式调用已验证。Desktop picker 的 `/cotend` 查询、热更新、七条列表入口和五个主要友好显示名已部分验证；详情、其他查询/排序、token 插入、自然语言发现和最小运行时架构仍在后续端到端验证前保持待验证。
...
当前进度：渠道中立的项目级底层已实现 inspect、install、update、repair、`migrate_identity`、enable、disable、uninstall、rollback 和 candidate-free recover，并验证 dry-run、精确所有权、schema v1 receipt 迁移、降级阻止、受控异常注入后的自动恢复、项目级并发排他、强制终止检测、snapshot-bound recovery confirmation、遗留 pre-write lock 安全释放、已验证 checkpoint 回滚及无关文件保护。当前 recover 只开放两个保守分支，active/unknown owner、无效证据、unexpected content 和缺少 intended-target 证明的 forward finalize 仍阻断。三层渠道职责已确认；一个不可发布的 skills-only Plugin fixture 已通过 17 步隔离 local Marketplace/Plugin Phase A、7+7 namespaced/standalone 共存和 12 类负向验证。namespace 重验进一步完成两个独立物理候选的各 10 步 lifecycle：preserve 为零 Skill 字节改写，short 虽获得 `cotend:init` 等入口，却需要 28 个路径移动、10 个文件改写并保留 60 处非协议语义复核债务；用户已确认 display-led preserve-first 为下一 production-candidate 基线，后续 `/cotend` Desktop picker 已证明热更新、七条列表和五个主要友好显示名。CLI 与 fixture 仍只是开发适配器和 disposable 证据，不是正式 Plugin、Public Directory 包或 novice 安装体验；cachebuster update、enable/disable、Desktop 详情/其他查询/token 插入、新任务刷新、真实项目、模型委派、断电/网络文件系统、用户/全局范围、平台迁移和全部 recovery apply 尚未验证，因此 P6 Gate 未关闭。
...
- Desktop 技能选择器的 `/cotend` 热更新、七条列表标题和可见说明截断已验证；详情、其他查询/排序、canonical placement、自然语言触发和点击后的 token 插入仍待验证。项目级 CLI 发现与显式调用已验证。
...
- 正式 Plugin manifest 名称/版本/namespace、共享规则运行时边界和最终小白安装实现；不可发布 fixture 已证明 7 Skills 可打包、安装、共存、移除和重装，并执行比较 `cotend:cotend-init` 与 `cotend:init` 两类 namespace。display-led preserve-first 已确认为下一候选基线，`/cotend` picker 已支持其五个主要友好显示名；最终双前缀仍待详情/canonical placement、token 插入与模型介导委派证据，真实项目及可写 live 旅程尚未完成。
- 项目状态的最小物理布局。
- 各渠道的具体构建、安装、升级、审核和公开发布执行顺序。
