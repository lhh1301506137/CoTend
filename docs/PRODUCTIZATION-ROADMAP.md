...
当前进度：仓库源树导入、CoTend 改名、来源与许可证记录、C01-C19 映射、确定性项目级 fixture、Codex 实际发现和三条只读显式调用场景已完成；真实可写本地项目、完整旅程和 live 安装尚未完成，因此 P4 Gate 仍未关闭。
...
- package/plugin 载体、共享规则运行时边界和安装生命周期。
- 项目状态的最小物理布局。
- 安装渠道与公开发布顺序。
