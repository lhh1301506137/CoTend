...
- 正式 Plugin manifest 名称/版本/namespace、共享规则运行时边界和最终小白安装实现；不可发布 fixture 已证明 7 Skills 可打包、安装、共存、移除和重装，并执行比较 `cotend:cotend-init` 与 `cotend:init` 两类 namespace。display-led preserve-first 只是待确认推荐，Desktop 显示/搜索与模型介导委派、真实项目及可写 live 旅程尚未完成。
- 项目状态的最小物理布局。
