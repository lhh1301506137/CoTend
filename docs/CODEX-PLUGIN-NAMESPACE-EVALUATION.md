...
```yaml
status: user_confirmed_candidate_baseline_with_new_task_picker_and_chip_evidence
evidence_basis: executed_two_physical_candidates_plus_controlled_refresh_and_desktop_inspection
...
| N3 display-led | canonical 与 N1 相同；五个用户原创 Skill 显示为 `CoTend Init` 等 | 复用 N1，新增 0 bytes | 保留单一语义源；`/cotend` 已显示全部 7 个入口，新任务可插入 `CoTend Init` 友好 chip | 已打开任务不能可靠热更新；两个 MIT companion 显示平台生成的 `Cotend: ...`；详情、其他查询/排序和模型委派未验证 | metadata/fresh discovery `executed`；Desktop picker/chip `executed_user_assisted/inspection`；其余交互 `not_run` |
...
app-server 已确认 canonical name 是 `cotend:cotend-init`，不能继续假设 Plugin 精确入口仍叫 `$cotend-init`。Desktop 实证表明，用户在选择器输入 `/cotend` 时能看到全部 7 个入口；五个用户原创 Skill 主要显示 `CoTend Init`、`CoTend Diagnose Only` 等友好元数据，两个 MIT companion 显示为平台生成的 `Cotend: ...`。新空任务还能在未发送编辑器中插入 `CoTend Init` 友好 chip，截图内没有出现 canonical 双前缀。

先前用 `$CoTend` 查询得到的不可见结果仍是错误语法造成的假阴性，但后续受控重装证明另一条独立边界：同一已打开任务不会可靠刷新新增 Skill，fresh app-server 能发现 7 项，新空任务随后可见。首次 cleanup 后仍显示列表的截图兼容 stale task snapshot，不能作为 add-hot-update 证据。当前证据仍没有展示详情页中 canonical name 的位置，也没有验证其他搜索词或排序，因此仅凭 canonical 字符串较长就承担完整短名迁移成本，证据仍不足。
...
- Desktop picker 与新任务 chip 已证明五个主要入口的显示元数据足以支持 `/cotend` 识别和非发送式选择，因此当前不需要改共享 Skill ID；
- 若 Desktop 证明 canonical 双前缀仍直接造成明显混淆，再把短名迁移作为上游变更提案，而不是在 CoTend package 中先行分叉。

## 推荐

用户已确认 **N3 display-led preserve-first** 作为下一步 production-package 设计的候选基线。后续 `/cotend` 列表、新任务刷新与友好 chip 实证支持继续采用该候选，但仍不是最终 namespace 定案；剩余详情、查询与模型证据可以继续否证该候选，若否证成立再进入上游短名或兼容层提案。
...
3. `/cotend` 列表标题、可见截断、新任务刷新和非发送友好 chip 已经实证；安装/更新后必须提示打开新任务或执行等价 Skill 快照刷新。确认正式 namespace 前仍需验证详情、其他查询/排序和 canonical placement。
...
- 正式 Plugin ID 与公开版本；
- canonical 双前缀在详情中是否会让普通用户困惑；新任务 chip 当前只显示友好名称；
- display name 的最终英文文案；
- 是否需要平台支持的 alias，而不是改 Skill ID；
- Desktop 与自然语言证据不通过时，采用上游短名迁移还是其他兼容层；
- production candidate 已由 L44 固定，并由 L46 证明精确包在隔离 CLI 中可安装、发现、移除和故障清理；真实个人安装、完整 Desktop 生命周期、公开提交和发布授权仍开放。
...
- `/cotend` 七条列表、新任务刷新和 `CoTend Init` 非发送 chip 已验证；当前任务热更新未通过。下一优先级是详情、其他查询/排序、canonical placement 和模型委派，通过前不确认最终双前缀。
- Desktop 或模型证据若否证 N3，应形成上游变更提案，不在 CoTend package 中先行分叉共享行为。
- L44 已使用 N3 字节建立 `cotend@0.1.0-rc.1` 的未发布生产候选包契约；它保持 `codex-skills/` 为唯一语义源，并不确认最终 namespace 或正式版本。
- L46 的正常与故障恢复场景都观察到 `cotend:<skill>` namespace，并保持 Plugin 与 standalone Skill 共存隔离；该结果证明运行时一致性，不证明普通用户已经接受双前缀。
