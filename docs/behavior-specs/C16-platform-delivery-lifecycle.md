...
- exact current artifact identity and, when a candidate is required, its source, provenance, license, integrity, compatibility, and C15 applicability;
...
3. Select the least-assumptive operation: `current_no_change`, `install`, `enable`, `update`, `repair_delivery`, `migrate_platform`, `disable`, `uninstall`, or `recover_delivery`. Do not turn a project repair into a product reinstall or vice versa.
...
- candidate relation: `none`, `install_candidate`, `same_as_current`, `update_available`, `incompatible`, or `unknown`;
...
- `current_no_change` leaves a verified stable vector unchanged, performs no acquisition or mutation, and preserves the explicitly inspected candidate relation: `none`, `same_as_current`, or an `update_available` candidate whose update was explicitly deferred;
...
- `update` requires candidate relation `update_available`, moves that exact candidate through acquisition and staging to a new `complete` identity, verifies enablement and invocation, retains the prior checkpoint, and ends with candidate relation `same_as_current`;
- `repair_delivery` uses the exact trusted current or reconstruction candidate to move a `partial` or `damaged` installation, or a failed enablement or invocation state, to one verified stable vector without changing project lifecycle truth; the repaired identity ends with candidate relation `same_as_current`;
