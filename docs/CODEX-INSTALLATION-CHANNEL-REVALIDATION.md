...
| 更新与卸载 | 隔离 adapter 已实现整套 7 Skill 的 component receipt、更新、修复、禁用、卸载、rollback 和 recovery；真实 user apply 与 production state resolver 仍待验证。 | 平台提供 install/enable/disable/remove 与版本化缓存；本地开发更新仍需 cachebuster、reinstall 和新任务。 |
| 小白前置知识 | 手工复制目录不合格；由 AI 调用 installer 可降低门槛，但仍是本地 setup/实验语义。 | 公共 Directory 支持桌面搜索、详情和点击安装，是当前最明确的公开 novice 路径。 |
| 当前完成度 | 隔离 user-scope adapter 已复用同一 transaction core 并通过 19 项 lifecycle/negative/recovery 测试；真实用户 apply 与 production state root 仍未验证。 | 隔离 fixture、真实临时 add/remove 和部分 Desktop 表面已验证；production manifest/version/submission 仍未完成。 |
...
| Direct user Skills | 个人 Early Access、本地/离线与修复 | 若由 AI installer 执行可不要求手工目录知识；手工复制仍不合格 | CoTend 隔离 adapter 已覆盖组件级 install/update/repair/disable/uninstall/rollback/recovery；真实用户 apply 仍关闭 | 仅个人 | 已确认为次级渠道，等待 production resolver 与真实验证 |
...
user_receipt_schema: 3
isolated_test_result: 19_passed_0_skipped
```
...
真实 user scope apply、production state root、同一 user roots 到唯一 state/lock root 的稳定解析、当前机器重复 companion 清理、Desktop 发现、Plugin 共存迁移和公开发布仍是独立停止门；本次实现不得被描述为用户级安装已经上架或可直接运行。
...
只读 resolver 为 canonical installation 和当前双根 layout 分别生成摘要身份。同 HOME 使用多个 `CODEX_HOME` 时，installation ID 与 state root 不变，layout fingerprint 改变；传入先前 fingerprint 时会明确报告 `layout_context_changed`。旧 user receipt schema v3、未知 state、canonical 首方残留和 compatibility 首方残留均只报告显式 blocker；不自动迁移、删除或接管。

non-live CLI 接受现有 lifecycle operation 用于显示预览，但当前没有 production manager。任何 `--apply` 在解析 HOME、扫描 Skills 或读取 state 之前返回 `production_apply_forbidden`。13 项隔离测试和六项真实 stat-only 边界快照通过，详见 [`docs/evidence/PRODUCTION-USER-LAYOUT-RESOLVER.md`](evidence/PRODUCTION-USER-LAYOUT-RESOLVER.md)。

下一步仍需单独设计 production receipt 身份和 schema v3 migration plan。真实 user apply、Desktop 发现、Plugin 共存和公开发布未获授权，也未被本验证证明。
