...
CoTend 已有一个纯只读 production user layout resolver 和 non-live CLI。它可以确定 canonical、compatibility 与 state 路径，生成不暴露原始路径的 installation ID 和 layout fingerprint，并报告旧 receipt、schema v4 envelope、未知 state 与首方残留的显式迁移阻塞。当前构建没有 production mutator；CLI 收到 `--apply` 时会在路径解析和文件扫描之前返回 `production_apply_forbidden`。
...
- 发现 schema v4 时只做 envelope 深度的 installation/layout identity 分类；完整 receipt、payload 和 component 验证由共享 transaction core 执行；
- schema v4 installation ID 不一致时报告 `blocked_installation_identity_mismatch`，layout fingerprint 不一致时报告 `explicit_layout_context_migration_required`；
...
resolver 的 schema v3/v4 检查都只识别 envelope，用于路由到显式迁移或阻塞；它不等同于完整验证 receipt 与 payload。后续隔离 schema v4 与完整 v3 migration 证据见 [`ISOLATED-PRODUCTION-USER-RECEIPT.md`](ISOLATED-PRODUCTION-USER-RECEIPT.md)。
...
本叶及后续隔离实现都没有修改 project receipt schema v1/v2、isolated user receipt schema v3、`codex-skills/`、target lock 或 framework lock。resolver 始终只读；后续 schema v4 可写核心仅能由显式 isolation-root 测试适配器构造。

## 未证明事项

- schema v4 与 schema v3 完整迁移已在隔离 fixture 证明，但真实 production bridge 仍硬关闭；
