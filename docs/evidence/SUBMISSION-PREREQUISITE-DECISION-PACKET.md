...
| --- | --- | --- | --- |
| 1 | Q01 Publisher mode | `verified_publisher_identity` | 唯一等待用户回答 |
| 2 | Q02 Final Plugin identity | `final_plugin_identity_and_version` | 被 Q01 阻塞 |
| 3 | Q03 Public web presence | `website_url`、`support_url`、`privacy_policy_url`、`terms_url` | 被 Q01/Q02 阻塞 |
...
Q01 决定身份路径，但选择个人或企业并不等于身份已经验证；对应 blocker 只有在 OpenAI Platform 出现真实验证证据后才能关闭。同理，选择公开页面路线不等于四个 URL 已经上线。
...
## 当前问题：Q01 Publisher Mode

**问题**：CoTend 首次提交应使用个人身份还是企业身份完成发布者验证？

1. **推荐：个人身份首发**。准备项通常更少，但提交材料中的开发者名称和公开信息需要与已验证个人身份一致，可能暴露个人名称。选择该项只确定路线，不授权现在登录 Platform 或公开个人信息。
2. **企业身份首发**。品牌与个人身份分离更清晰，但需要可验证的企业身份、对应 organization 权限和一致的公开法律信息。
3. **暂缓决定**。保持全部依赖项阻塞，不进行身份验证、公开页面定案或 Portal 操作。
...
```text
SUBMISSION_PREREQUISITES_OK status=awaiting_owner_decisions prerequisites=10 decisions=7 active=Q01-publisher-mode digest=e23febd663c4abd82c7de2a2afde5ccd7599454c141669e238b8d1a336a6f066
Ran 8 tests - OK
Ran 145 tests - OK
