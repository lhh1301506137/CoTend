...
- `executed`：安装前 dry-run 没有改变 fixture；显式 apply 后 receipt 精确记录 release、protocol、7 个 Skill、30 个文件和 manifest identity。
...
- 24/24 单元测试通过，其中 21 项为 L24 交付核心回归，3 项覆盖 L25 receipt、共存模式和发现契约。
- 原 L21 严格模式仍默认要求精确 7 Skill；4 类负向和 `skills/list` 7/7 发现通过。允许无关 Skill 只有 L25 显式传入时生效。
- L24 的 11 步正向和 6 类负向生命周期通过。
...
没有直接修改 `cotend-diagnose-only`：当前 target artifact ID 仍是 `2026.07.11.3`。确定性假想补丁证明，只改变该 Skill 字节而沿用同一 ID 会被交付核心分类为 `incompatible` / `artifact_identity_conflict`。修复必须先解决两件事：由 upstream 审查通用的语言优先级/adapter fallback 语义，以及由 CoTend 确认 source release 与 target artifact revision 的独立身份；不能通过显式 English 测试 prompt 或同 ID 重发字节制造假通过。
