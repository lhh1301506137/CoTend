...
仓库内 `skills/` 源码载体已经采用，项目级 `.agents/skills` 载体也已通过当前 Codex 的发现与只读显式调用验证。渠道职责已确认：项目 Skills 服务开发回归，GitHub repo Marketplace 服务 Open Beta，Public Plugins Directory 是后续正式目录目标。仓库根 Plugin + Marketplace 已在全隔离进程环境通过 local Marketplace add/list、Plugin install/list/remove/reinstall、app-server discovery、standalone 共存和失败恢复；实际安装入口为 `cotend:<skill>`，真实 `owner/repo` 网络安装仍需首次 push 后验证。

后续 namespace 重验又实际比较了 preserve 与 package-time short 两个独立候选。preserve 保持 30 个 Skill 文件逐字节一致并暴露 `cotend:cotend-init`；short 暴露 `cotend:init`，但需要移动 28 个路径、改写 10 个文件，仍留下 60 处非协议引用需要语义复核。用户已确认 display-led preserve-first 作为下一 production-candidate 基线：复用 preserve bytes 和已有友好显示元数据。Desktop 证据证明 `/cotend` 可显示全部 7 个入口，五个用户原创 Skill 使用友好显示名，新空任务可插入 `CoTend Init` 友好 chip；两个 MIT companion 使用平台生成的 `Cotend: ...` 前缀。受控重装同时证明已打开任务不会可靠刷新新增 Skill，因此安装/更新后必须暂按“打开新任务或执行等价 Skill 快照刷新”设计。详见 [`docs/CODEX-PLUGIN-NAMESPACE-EVALUATION.md`](CODEX-PLUGIN-NAMESPACE-EVALUATION.md)、[`docs/evidence/CODEX-PLUGIN-NAMESPACE-CANDIDATES.md`](evidence/CODEX-PLUGIN-NAMESPACE-CANDIDATES.md) 和 [`docs/evidence/CODEX-DESKTOP-PLUGIN-SURFACE.md`](evidence/CODEX-DESKTOP-PLUGIN-SURFACE.md)。这不确认正式 Plugin manifest、公开版本、最终 namespace、项目状态物理布局、详情/其他查询、模型委派或用户级 production 安装。它们继续从已采用 release 的实际结构和最小产品化增量出发，再用行为契约、真实用户情境和平台能力验证；[`docs/REFERENCE-FRAMEWORK-IMPLEMENTATION-STUDY.md`](REFERENCE-FRAMEWORK-IMPLEMENTATION-STUDY.md) 只提供二级参考，不构成运行时、状态布局或安装方案的实现权威。
