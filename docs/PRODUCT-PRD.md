...
release 角色盘点和用户确认已经确定仓库源树分层：`cotend-init` 是统一可见入口，`cotend-project-init` 是内部 Auto Mode 引擎，`cotend-collaboration` 是共享治理核心，诊断与模型升级保持情境/进阶入口，两个 MIT Skill 保持内部伴随角色。五个用户原创 Skill 使用统一 `cotend-` 前缀；这确认的是源码角色与名称，不是未经 live Codex 验证的菜单排序或自然语言发现效果。

仓库内 `codex-skills/` 源码载体已经采用；运行时与打包架构、项目状态的物理布局、live 安装后实际可见性，以及安装和分发渠道仍未确认。它们继续从已采用 release 的实际结构和最小产品化增量出发，再用行为契约、真实用户情境和平台能力验证；[`docs/REFERENCE-FRAMEWORK-IMPLEMENTATION-STUDY.md`](REFERENCE-FRAMEWORK-IMPLEMENTATION-STUDY.md) 只提供二级参考，不构成运行时、状态布局或安装方案的实现权威。
...
行为契约按波次审查并由用户确认；当前波次和确认门只以 [`docs/BEHAVIOR-SPEC-INDEX.md`](BEHAVIOR-SPEC-INDEX.md) 为准，本 PRD 不复制动态状态。仓库 Codex Skill 的 7 目录分层、五个 CoTend 名称和两个第三方原名已经确认并采用；I6 与历史 5+1 仍只是历史界面候选。当前没有经 live Codex 验证的活动调用界面基线，运行时架构、状态布局和安装渠道也仍待确认。
...
- 仓库 Codex Skill 的角色、名称和第三方 bundling 已确认并采用；真实调用可见性、架构和项目状态仍待验证或确认。
- 首发产品语言已确认为英文。
