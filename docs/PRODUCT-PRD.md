...
release 角色盘点和用户确认已经确定仓库源树分层：`cotend-init` 是统一可见入口，`cotend-project-init` 是内部 Auto Mode 引擎，`cotend-collaboration` 是共享治理核心，诊断与模型升级保持情境/进阶入口，两个 MIT Skill 保持内部伴随角色。五个用户原创 Skill 使用统一 `cotend-` 前缀。项目级 `.agents/skills` 发现、`$cotend-init`/`$cotend-diagnose-only` 显式调用、内部委派和关键停止边界已经通过隔离 Codex 验证；后续 `/cotend` Desktop picker 又验证了热更新、七条列表入口和五个主要友好显示名。这仍不等于详情、其他搜索词/排序、token 插入或自然语言发现效果已经确认。
...
行为契约按波次审查并由用户确认；当前波次和确认门只以 [`docs/BEHAVIOR-SPEC-INDEX.md`](BEHAVIOR-SPEC-INDEX.md) 为准，本 PRD 不复制动态状态。仓库 Codex Skill 的 7 目录分层、五个 CoTend 名称和两个第三方原名已经确认并采用；I6 与历史 5+1 仍只是历史界面候选。项目级 `$skill-name` 显式调用已有真实 Codex 证据，Desktop picker 的 `/cotend` 查询、热更新和七条列表也已验证，但尚不足以形成包含详情、其他查询/排序、token 插入、隐式发现和安装体验的完整界面基线；运行时架构、状态布局和最终安装渠道也仍待确认。
...
- 仓库 Codex Skill 的角色、名称和第三方 bundling 已确认并采用；项目级 CLI 发现、显式调用和三项关键行为已验证，Desktop picker 的 `/cotend` 热更新与七条列表已部分验证；详情、其他查询/排序、token 插入、隐式发现、架构和项目状态仍待验证或确认。
- 首发产品语言已确认为英文。
