...
行为契约按波次审查并由用户确认；当前波次和确认门只以 [`docs/BEHAVIOR-SPEC-INDEX.md`](BEHAVIOR-SPEC-INDEX.md) 为准，本 PRD 不复制动态状态。仓库 Codex Skill 的 7 目录分层、五个 CoTend 名称和两个第三方原名已经确认并采用；I6 与历史 5+1 仍只是历史界面候选。项目级 `$skill-name` 显式调用已有真实 Codex 证据，但尚不足以形成包含 Desktop 菜单、隐式发现和安装体验的完整界面基线；运行时架构、状态布局和安装渠道也仍待确认。
...
- 仓库 Codex Skill 的角色、名称和第三方 bundling 已确认并采用；项目级 CLI 发现、显式调用和三项关键行为已验证，Desktop/隐式发现、架构和项目状态仍待验证或确认。
- 首发产品语言已确认为英文。
