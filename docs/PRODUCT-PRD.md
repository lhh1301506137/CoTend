...
notice_state: complete_for_initial_codex_source_adoption
...
interface_revalidation_reason: desktop_selector_and_implicit_discovery_not_validated
...
这些契约现在是产品化覆盖和验收护栏：直接改名或适配不得静默丢失任何已确认能力，但契约本身不要求把 dual-ai 重写成一套新流程。首个 CoTend adoption 记录已经建立，C01-C19 已映射到 7 个仓库内 Codex Skill；live 安装与交付生命周期仍需后续验证。
...
release 角色盘点和用户确认已经确定仓库源树分层：`cotend-init` 是统一可见入口，`cotend-project-init` 是内部 Auto Mode 引擎，`cotend-collaboration` 是共享治理核心，诊断与模型升级保持情境/进阶入口，两个 MIT Skill 保持内部伴随角色。五个用户原创 Skill 使用统一 `cotend-` 前缀。项目级 `.agents/skills` 发现、`$cotend-init`/`$cotend-diagnose-only` 显式调用、内部委派和关键停止边界已经通过隔离 Codex 验证；后续 `/cotend` Desktop picker 又验证了七条列表入口、新任务刷新、五个主要友好显示名和 `CoTend Init` 非发送 chip。已打开任务的热更新未通过，详情、其他搜索词/排序和自然语言发现仍未确认。

仓库内 `skills/` 源码载体已经采用，项目级 `.agents/skills` 载体也已通过当前 Codex 的发现与只读显式调用验证。渠道职责已确认：项目 Skills 服务开发回归，GitHub repo Marketplace 服务 Open Beta，Public Plugins Directory 是后续正式目录目标。仓库根 Plugin + Marketplace 已在全隔离进程环境通过 local Marketplace 生命周期；首次普通 push 后，又从真实 `lhh1301506137/CoTend` 完成远端 add/list、同 revision Git-backed refresh、Plugin install/remove、7 个 `cotend:<skill>` 发现、失败恢复和清理。该结果开放 GitHub Open Beta CLI 路径，但不等于真实用户写入、跨版本升级、完整 Desktop 生命周期、稳定版或 Public Plugins Directory 上架。

后续 namespace 重验又实际比较了 preserve 与 package-time short 两个独立候选。preserve 保持 30 个 Skill 文件逐字节一致并暴露 `cotend:cotend-init`；short 暴露 `cotend:init`，但需要移动 28 个路径、改写 10 个文件，仍留下 60 处非协议引用需要语义复核。用户已确认 display-led preserve-first 作为 production-candidate 基线：复用 preserve bytes 和已有友好显示元数据；它现在也是 `cotend@0.1.0-rc.1` GitHub Open Beta 的实际 namespace。Desktop 证据证明 `/cotend` 可显示全部 7 个入口，五个用户原创 Skill 使用友好显示名，新空任务可插入 `CoTend Init` 友好 chip；两个 MIT companion 使用平台生成的 `Cotend: ...` 前缀。受控重装同时证明已打开任务不会可靠刷新新增 Skill，因此安装/更新后必须暂按“打开新任务或执行等价 Skill 快照刷新”设计。详见 [`docs/CODEX-PLUGIN-NAMESPACE-EVALUATION.md`](CODEX-PLUGIN-NAMESPACE-EVALUATION.md)、[`docs/evidence/CODEX-PLUGIN-NAMESPACE-CANDIDATES.md`](evidence/CODEX-PLUGIN-NAMESPACE-CANDIDATES.md) 和 [`docs/evidence/CODEX-DESKTOP-PLUGIN-SURFACE.md`](evidence/CODEX-DESKTOP-PLUGIN-SURFACE.md)。这不确认最终稳定版或 Public Directory 的 manifest/version/namespace，也不确认项目状态物理布局、详情/其他查询、模型委派或用户级 production 安装。它们继续从已采用 release 的实际结构和最小产品化增量出发，再用行为契约、真实用户情境和平台能力验证；[`docs/REFERENCE-FRAMEWORK-IMPLEMENTATION-STUDY.md`](REFERENCE-FRAMEWORK-IMPLEMENTATION-STUDY.md) 只提供二级参考，不构成运行时、状态布局或安装方案的实现权威。
...
行为契约按波次审查并由用户确认；当前波次和确认门只以 [`docs/BEHAVIOR-SPEC-INDEX.md`](BEHAVIOR-SPEC-INDEX.md) 为准，本 PRD 不复制动态状态。仓库 Codex Skill 的 7 目录分层、五个 CoTend 名称和两个第三方原名已经确认并采用；I6 与历史 5+1 仍只是历史界面候选。项目级 `$skill-name` 显式调用已有真实 Codex 证据，Desktop picker 的 `/cotend` 七条列表、新任务刷新和友好 chip 也已验证；当前任务热更新未通过，且尚不足以形成包含详情、其他查询/排序、隐式发现和安装体验的完整界面基线。运行时架构、状态布局和最终安装渠道也仍待确认。
...
- 仓库 Codex Skill 的角色、名称和第三方 bundling 已确认并采用；项目级 CLI 发现、显式调用和三项关键行为已验证，Desktop picker 的 `/cotend` 七条列表、新任务刷新与友好 chip 已部分验证；当前任务热更新未通过，详情、其他查询/排序、隐式发现、架构和项目状态仍待验证或确认。
- 首发产品语言已确认为英文。
- 面向产品 owner 的分析、研究、评估和审查说明正文默认使用中文；英文入口与首发产品语言保持不变。
- 来源、许可证、隐私和受限材料隔离成为导入与发布前不可绕过的约束。
- 根目标、MVP 和产品化路线已纳入公开产品基线。
- `ready_for_behavior_specification: yes`；`ready_for_repository_source_implementation: yes`；`ready_for_live_delivery: no`，直到安装、真实 Codex 触发和生命周期验证就绪。
